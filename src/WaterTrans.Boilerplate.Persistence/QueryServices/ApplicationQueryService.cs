...
{
    public class ApplicationQueryService : SqlQueryService, IApplicationQueryService
    {
        public ApplicationQueryService(IDBSettings dbSettings)
            : base(dbSettings)
        {
        }
        ...
        public Domain.Entities.Application GetByClientId(string clientId)
        {
            ...
            Domain.Entities.Application result = Connection.Query<ApplicationSqlEntity>(
                sql.ToString(), param, commandTimeout: DBSettings.CommandTimeout).SingleOrDefault();
            return result;
        }
